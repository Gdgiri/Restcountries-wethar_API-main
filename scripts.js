  ...
  .then((result) => {
    ...
    buttons.forEach((btn, index) => {
      btn.addEventListener("click", () => {
        ...
        // ? weather api logic
        let wetharapi = fetch(
          `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=27841d2b206619c6fc9c3093a252883c`
        );
        wetharapi
          ...
          .then((ele22) => {
            ...
            console.log(ele22);
            alert(`Weather of ${result[index].name.common} is : ${Math.floor(
              ele22.main.temp
            )}
            °C`);
          });
